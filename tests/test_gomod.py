...
from common.packages_data import _package_sort_key
...
# from tasks.gomod import gomod
from tasks.gomod import (
...
@mock.patch("tasks.gomod._get_golang_version")
@mock.patch("tasks.gomod.GoCacheTemporaryDirectory")
@mock.patch("tasks.gomod._merge_bundle_dirs")
@mock.patch("tasks.gomod._get_allowed_local_deps")
@mock.patch("tasks.gomod._vet_local_deps")
@mock.patch("tasks.gomod._set_full_local_dep_relpaths")
@mock.patch("common.config.get_worker_config")
...
def test_resolve_gomod(
    ...
    request = Request(url="", ref="c50b93a32df1c9d700e3e80996845bc2e13be848")
    if cgo_disable:
    ...
    if dep_replacement is None:
        gomod = _resolve_gomod(archive_path, request)
        expected_deps = sample_deps
    else:
        gomod = _resolve_gomod(archive_path, request, [dep_replacement])
    ...
    mock_merge_tree.assert_called_once_with(
        ...
        str(RequestBundleDir(request.source_dir).gomod_download_dir),
    )
...
@mock.patch("tasks.gomod.GoCacheTemporaryDirectory")
@mock.patch("subprocess.run")
@mock.patch("common.paths.RequestBundleDir")
@mock.patch("tasks.gomod._module_lines_from_modules_txt")
...
@mock.patch("subprocess.run")
@mock.patch("tasks.gomod._get_golang_version")
@mock.patch("tasks.gomod.get_worker_config")
...
def test_resolve_gomod_strict_mode_raise_error(
    ...
    archive_path = "/this/is/path/to/archive.tar.gz"
    request = Request(ref="c50b93a32df1c9d700e3e80996845bc2e13be848", url="")
    expected_error = (
...
@mock.patch("tasks.gomod._merge_files")
...
@mock.patch("tasks.gomod._fail_unless_allowed")
...
def test_should_vendor_deps(flags, vendor_exists, expect_result, tmp_path):
    ...
    assert _should_vendor_deps(flags, str(tmp_path), False) == expect_result
...
def test_should_vendor_deps_strict(flags, vendor_exists, expect_error, tmp_path):
    ...
    if expect_error:
        ...
        with pytest.raises(ValidationError, match=msg):
            _should_vendor_deps(flags, str(tmp_path), True)
    else:
        _should_vendor_deps(flags, str(tmp_path), True)
...
@mock.patch("tasks.gomod._run_download_cmd")
@mock.patch("tasks.gomod._vendor_changed")
...
def test_module_lines_from_modules_txt(tmp_path):
    ...
    assert _module_lines_from_modules_txt(str(tmp_path)) == [
        "github.com/org/some-module v0.0.0 => ./example/src/some-module",
        "golang.org/x/text v0.0.0-20170915032832-14c0d48ead0c",
        "rsc.io/quote v1.5.2 => rsc.io/quote v1.5.1",
        "rsc.io/sampler v1.3.0",
...
def test_module_lines_from_modules_txt_invalid_format(file_content, expect_error_msg, tmp_path):
    ...
    with pytest.raises((InvalidFileFormat, FileAccessError), match=expect_error_msg):
        _module_lines_from_modules_txt(str(tmp_path))
